...
public class Carrinho implements Serializable{
	...
	/**
	 * M�todo que adiciona itens do cardapio no carrinho
	 * 
	 * REGRAS : No mesmo carrinho n�o � permitido misturar itens de restaurantes diferentes.
	 ...
	 *          No carrinho n�o pode ser inserido o mesmo elemento.
	 * 
	 * @param itemCardapio
	 * @param quantidade
	 * @param observacoes
	...
	public void adicionarItem(ItemCardapio itemCardapio, Integer quantidade, String observacoes) throws RestauranteDiferenteException{
		...
		if(!exists(itemCardapio)) { // Se n�o existe o item de cardapio no carrinho, ele � ADICIONADO
			...
			// O pre�o do item do cardapio pode mudar, mas o pre�o do item do pedido tem que se manter da epoca da compra ent�o essa informa��o e necessaria, pois o pre�o do pedido n�o pode ser alterado
	 ...
	 * Como ja e feita toda a valida��o no m�todo adicionarItem() nao � necessario tratar exce��o aqui
	 * @param itemPedido
	 */
	...
	public void removerItem(ItemCardapio itemCardapio) {
		...
		for (Iterator<ItemPedido> iterator = itens.iterator(); iterator.hasNext();) {
			...
			if(itemPedido.getItemCardapio().getId().equals(itemCardapio.getId())) {
				iterator.remove();
				break; // Ja que n�o possui elementos de item de cardapio repetidos assim que ele encontrar o item n�o existe a necessidade de continuar procurando
			}
		}
	...
	private boolean exists(ItemCardapio itemCardapio) {
		...
		return false; // O item de cardapio ainda n�o existe no carrinho, retorna false
	}
	...
	/**
	 * Metodo (Atalho) que indentifica se o carrinho esta vazio ou n�o 
	 * @return
	 */
